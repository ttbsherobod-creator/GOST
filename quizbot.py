...
def check_access(uid):
    ...
    row = cur.execute("SELECT blocked, allowed FROM users WHERE user_id=?", (uid,)).fetchone()
    conn.close()
...
async def send_q(update, context):
    ...
    if i >= 30:
        ...
        await update.effective_chat.send_message(f"🏁 Natija: {score}/30")
        return
...
async def stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn = sqlite3.connect("bot.db")
    row = conn.execute("SELECT name,tests,correct,last_active FROM users WHERE user_id=?", (uid,)).fetchone()
    conn.close()
